...
def read_state(session_id: str) -> dict[str, Any]:
    state = execute(session_id, """
        ...
        const images=[...document.images]; const skip=document.querySelector('.skip-link');
        ...
        return {ready:document.readyState,title:document.title,width:window.innerWidth,height:window.innerHeight,
          ...
          canonical:document.querySelector('link[rel="canonical"]')?.href||'',controlCount:controls.length,
          minControlHeight:controls.length?Math.min(...controls.map(node=>rect(node).height)):0,styles,
          brokenImages:images.filter(img=>!img.complete||img.naturalWidth<=0).map(img=>img.getAttribute('src')),bodyRegions,skipLinkExists:Boolean(skip)};
    """)
...
def inspect_skip_link(session_id: str) -> dict[str, Any] | None:
    state = execute(session_id, """
        ...
        const rect=node=>{const r=node.getBoundingClientRect();return {left:r.left,right:r.right,top:r.top,bottom:r.bottom,width:r.width,height:r.height};};
        const before=rect(link); link.focus(); const after=rect(link); const active=document.activeElement===link; link.blur(); return {active,before,after};
    """)
    if state is None:
        return None
...
def exercise(session_id: str, url: str, expected_title: str, canonical_host: str) -> None:
    ...
    for requested_width, requested_height in VIEWPORTS:
        set_viewport(session_id, requested_width, requested_height); time.sleep(0.1); state = read_state(session_id)
        width = int(state.get("width", 0)); height = int(state.get("height", 0))
        ...
        require(state.get("glaze") == EXPECTED_GLAZE_VERSION, f"Simple-static Glaze version drift at {width}px: {state}")
        require(state.get("revision") == EXPECTED_GLAZE_REVISION, f"Simple-static Glaze revision drift at {width}px: {state}")
        require(state.get("consumerState") == EXPECTED_CONSUMER_STATE, f"Simple-static consumer-state boundary drift at {width}px: {state}")
        ...
        if state.get("skipLinkExists"):
            ...
            require(float(focused.get("left",-999))>=-1 and float(focused.get("right",99999))<=width+1 and float(focused.get("top",-999))>=-1 and float(focused.get("bottom",99999))<=height+1, f"Simple-static skip link is not viewport-contained when focused at {width}px: {skip}")


def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("site"); args = parser.parse_args(); spec = resolve_site(args.site)
    site = ROOT / spec.path; dist = site / "dist"
    require(dist.is_dir() and (dist / "index.html").is_file(), f"{spec.site_id} dist/ is missing; build exact artifact first")
    web_port = {"roadmap":8781,"blog":8782,"archive":8783}[spec.site_id]; url=f"http://127.0.0.1:{web_port}/"
    server: subprocess.Popen[bytes] | None=None; driver: subprocess.Popen[bytes] | None=None; session_id: str|None=None; log_path: Path|None=None
    try:
        server=subprocess.Popen(["python3","-m","http.server",str(web_port),"--bind","127.0.0.1","--directory",str(dist)],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL); wait_http(url)
        ...
        wait_for_driver(); session_id=create_session(); exercise(session_id,url,spec.title,spec.canonical_host)
        print(f"{spec.site_id} built-artifact Chrome acceptance passed at 1180×900, 768×900, 390×844, and 320×844."); return 0
    except Exception as error:
        ...
        if session_id:
            try: driver_request("DELETE",f"/session/{session_id}")
