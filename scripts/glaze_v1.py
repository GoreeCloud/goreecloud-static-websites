#!/usr/bin/env python3
"""Fetch the exact current Stable GLAZE UI V1.1 web bundle for same-origin static-site publication.

The immutable 1.1.0 Stable source has one known CSS import-closure defect:
`glaze-v1.components.css` imports a nonexistent `glaze-v1.candidate.css`.
Until a corrected immutable Stable release is published, controlled static-site
builds fail closed on that exact defect and remove only that single dangling
import from the generated artifact. Any other upstream drift remains a hard
failure. This workaround does not itself establish consumer conformance.
"""
...
KNOWN_STABLE_WORKAROUND_MARKER = (
    "GoreeCloud static-site build workaround: removed the single known dangling "
...
def _request(name: str) -> Request:
    return Request(
        f"{BASE_URL}/{name}",
        headers={"User-Agent": "GoreeCloud-Static-Websites-Build/1.1"},
    )
    ...
    for name, text in bundle.items():
        ...
        if name == KNOWN_STABLE_DEFECT_OWNER:
            if unexpected != {KNOWN_STABLE_DEFECT_IMPORT}:
                raise ValueError(
                    f"Known GLAZE V1.1 Stable import defect changed unexpectedly in {name}: {sorted(unexpected)}"
                )
            if text.count(KNOWN_STABLE_DEFECT_DIRECTIVE) != 1:
                raise ValueError("Known GLAZE V1.1 Stable dangling import must occur exactly once")
        elif unexpected:
