...
def set_viewport(session_id: str, width: int, height: int) -> None:
    request("POST", f"/session/{session_id}/goog/cdp/execute", {
...
def overflow_offenders(session_id: str) -> Any:
    return execute(session_id, """
        ...
        return [...document.querySelectorAll('body *')]
          ...
          .filter(x=>x.display!=='none'&&x.width>0&&(x.left < -1 || x.right > width + 1)).slice(0,20);
    """)
...
def layout_state(session_id: str) -> dict[str, Any]:
    state = execute(session_id, """
        ...
        const containers=['.site-header .shell','.hero','.release-band','.principle-grid','.surface-grid','.demo-grid','.adoption','footer .shell'].map(selector=>({selector,rect:rect(selector)}));
        return {ready:document.readyState,title:document.title,width:window.innerWidth,height:window.innerHeight,scrollWidth:document.documentElement.scrollWidth,bodyScrollWidth:document.body.scrollWidth,glaze:document.querySelector('meta[name="goreecloud-glaze-ui"]')?.content||'',revision:document.querySelector('meta[name="goreecloud-glaze-source-revision"]')?.content||'',consumerState:document.querySelector('meta[name="goreecloud-glaze-consumer-state"]')?.content||'',header:rect('.site-header'),nav:rect('.site-header nav'),theme:rect('.theme-group'),minNavHeight:nav.length?Math.min(...nav.map(el=>el.getBoundingClientRect().height)):0,minThemeHeight:themes.length?Math.min(...themes.map(el=>el.getBoundingClientRect().height)):0,brokenImages:images.filter(img=>!img.complete||img.naturalWidth<=0).map(img=>img.getAttribute('src')),principleColumns:columns('.principle-grid'),surfaceColumns:columns('.surface-grid'),demoColumns:columns('.demo-grid'),containers};
    """)
...
def validate_theme_controls(session_id: str) -> None:
    state = execute(session_id, """
        ...
        const light=click('light'); const dark=click('dark'); const system=click('system'); return {light,dark,system};
    """)
    require(isinstance(state, dict), f"Could not exercise Design Center theme controls: {state!r}")
    light, dark, system = state.get("light") or {}, state.get("dark") or {}, state.get("system") or {}
...
def exercise(session_id: str) -> None:
    ...
    for requested_width, requested_height in VIEWPORTS:
        ...
        require(state.get("title") == "GoreeCloud Design Center — GLAZE UI V1.4", f"Unexpected Design Center title at {width}px: {state}")
        require(state.get("glaze") == GLAZE_VERSION, f"Design Center Glaze version drift at {width}px: {state}")
        require(state.get("revision") == GLAZE_REVISION, f"Design Center Glaze revision drift at {width}px: {state}")
        require(state.get("consumerState") == CONSUMER_STATE, f"Design Center consumer-state boundary drift at {width}px: {state}")
...
def main() -> int:
    ...
    try:
        ...
        print("Design Center built-artifact Chrome acceptance passed at 1180×900, 768×900, 390×844, and 320×844 with GLAZE UI V1.4 appearance controls.")
        return 0
    except Exception as error:
