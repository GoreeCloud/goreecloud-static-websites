#!/usr/bin/env python3
"""Exact GLAZE UI V1.4 source/build contract for the GoreeCloud Main website."""
...
GLAZE_PROMOTION_REVISION = "84cb3db4884042f0fa25ed6d475a127fb110f596"
GLAZE_ENTRYPOINT = "glaze-v1.4.0.css"
GLAZE_ENTRYPOINT_BLOB = "d48a9bc317090d152799769271de0fb4325494c4"
GLAZE_CONSUMER_STATE = "build-migrated-rendered-acceptance-pending"
LEGACY_TEMPLATE_VERSION = "1.3.0"
LEGACY_TEMPLATE_REVISION = "8354308445da9ac35ced2b37a7f503a08a0aaf72"
CONSUMER_STYLESHEET = "css/glaze-v1.4-main.css"
...
def collect_glaze_css(root: Path) -> dict[str, bytes]:
    """Collect the exact same-origin CSS dependency closure rooted at V1.4 Stable."""
    lock = load_lock(root)
    ...
    def read(name: str) -> bytes:
        ...
        if name == GLAZE_ENTRYPOINT and git_blob_sha(data) != GLAZE_ENTRYPOINT_BLOB:
            raise ValueError("GLAZE UI V1.4 entrypoint integrity mismatch")
        return data
...
def render_v1_4_html(source: str) -> str:
    """Project legacy templates into the current V1.4 production contract.

    This is deliberately deterministic and narrow. It does not enable the optional
    V1.4 JavaScript Optical Engine or introduce any new context/telemetry collection.
    """
    rendered = source
    replacements = (
        (f'data-glaze-version="{LEGACY_TEMPLATE_VERSION}"', f'data-glaze-version="{GLAZE_VERSION}"'),
        (f'name="goreecloud-glaze-ui" content="{LEGACY_TEMPLATE_VERSION}"', f'name="goreecloud-glaze-ui" content="{GLAZE_VERSION}"'),
        (f'name="goreecloud-glaze-source-revision" content="{LEGACY_TEMPLATE_REVISION}"', f'name="goreecloud-glaze-source-revision" content="{GLAZE_PROMOTION_REVISION}"'),
        ('name="goreecloud-glaze-consumer-state" content="source-migrated-rendered-acceptance-pending"', f'name="goreecloud-glaze-consumer-state" content="{GLAZE_CONSUMER_STATE}"'),
        (f'css/glaze-v1.3.0.css" data-glaze-ui="{LEGACY_TEMPLATE_VERSION}"', f'css/{GLAZE_ENTRYPOINT}" data-glaze-ui="{GLAZE_VERSION}"'),
        ("GLAZE UI V1.3 source target", "GLAZE UI V1.4 source target"),
        ("official Stable GLAZE UI V1.3", "official Stable GLAZE UI V1.4"),
        (LEGACY_TEMPLATE_REVISION, GLAZE_PROMOTION_REVISION),
    )
    ...
    rendered = rendered.replace(
        ...
        '<header class="site-header glaze-material-soft" data-glaze-optical-v14="adaptive-optical">',
    )
    return rendered
