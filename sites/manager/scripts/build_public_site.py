...
if LOCK.get("version") != EXPECTED_VERSION or LOCK.get("lifecycle") != "Stable":
    raise SystemExit("Manager public-site Glaze lock must target 1.3.0 Stable")
if LOCK.get("stable_commit") != EXPECTED_COMMIT:
    raise SystemExit("unexpected Manager Glaze V1.3 source revision")
if LOCK.get("entrypoint") != EXPECTED_ENTRYPOINT or LOCK.get("entrypoint_blob") != EXPECTED_ENTRYPOINT_BLOB:
    raise SystemExit("unexpected Manager Glaze V1.3 entrypoint contract")
if LOCK.get("consumer_state") != "source-migrated-rendered-acceptance-pending":
    raise SystemExit("Manager public-site consumer state must remain fail-closed")
...
def read_glaze(name: str) -> bytes:
    ...
    try:
        if source_root:
            ...
            request = urllib.request.Request(url, headers={"User-Agent": "GoreeCloud-Manager-public-site-builder/1"})
    ...
    if name == EXPECTED_ENTRYPOINT and git_blob_sha(data) != EXPECTED_ENTRYPOINT_BLOB:
        raise SystemExit("Manager Glaze V1.3 entrypoint integrity mismatch")
    return data
...
for name in PUBLIC_FILES:
    shutil.copy2(require_file(ROOT / name), DIST / name)
...
for relative in LOCAL_ASSETS:
    ...
    shutil.copy2(source, DIST / "assets" / source.name)
...
print(
    ...
    "rendered, private-runtime, platform-integration, deployment, and production acceptance remain separate"
)
