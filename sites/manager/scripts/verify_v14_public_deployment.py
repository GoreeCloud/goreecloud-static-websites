#!/usr/bin/env python3
"""Verify the Manager GLAZE UI V1.4 public deployment against exact built bytes."""
...
DIST=ROOT/'dist'
BASE='https://manage.goreecloud.com/'
HOST='manage.goreecloud.com'
...
def fetch(path):
 ...
 req=Request(url,headers={'User-Agent':'GoreeCloud-Manager-V1.4-Verifier/1','Accept-Encoding':'identity','Cache-Control':'no-cache'},method='GET')
...
def validate_config():
 p=urlparse(BASE)
 if p.scheme!='https' or p.hostname!=HOST: raise SystemExit('Manager verifier must remain fixed to canonical HTTPS host')
 for rel in ('index.html','404.html','_headers','robots.txt','assets/manager-mark.svg'):
  if not (ROOT/rel).is_file(): raise SystemExit(f'missing Manager source: {rel}')
def verify():
 if not DIST.is_dir(): raise SystemExit('Manager dist missing; build first')
 errors=[]
 ...
 try:
  ...
  for marker in ('name="goreecloud-glaze-ui" content="1.4.0"',f'name="goreecloud-glaze-source-revision" content="{REV}"','name="goreecloud-glaze-consumer-state" content="build-migrated-rendered-acceptance-pending"','GLAZE UI V1.4 publication target'):
   if marker not in text: errors.append(f'/: missing V1.4 marker: {marker}')
 ...
 try:
  status,final,headers,body=fetch('/__goreecloud_manager_v14_missing__')
  if status!=404: errors.append(f'404: HTTP {status}, expected 404')
  expected=(DIST/'404.html').read_bytes()
  if body!=expected: errors.append('404: body differs from exact built 404.html')
 except Exception as e: errors.append(f'404: {e}')
 if errors:
  print('Manager V1.4 production verification failed:')
  for e in errors: print('- '+e)
  return 1
 print('Manager exact GLAZE UI V1.4 production publication verification passed; private Manager runtime acceptance remains separate')
 return 0
def main():
 p=argparse.ArgumentParser();p.add_argument('--check-config',action='store_true');p.add_argument('--target',choices=('production',))
 a=p.parse_args();validate_config()
 if a.check_config: print('Manager V1.4 verifier configuration valid; live acceptance remains separate');return 0
