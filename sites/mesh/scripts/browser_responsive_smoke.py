#!/usr/bin/env python3
"""Exercise the built Mesh Center at production-relevant viewport widths."""
...
def wait_http(url: str, timeout: float = 10) -> None:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            last = error
        time.sleep(0.1)
    raise BrowserError(f"local Mesh Center server did not become ready: {last}")
...
def exercise(session_id: str) -> None:
    ...
    for requested_width, height in VIEWPORTS:
        ...
        state = execute(
            ...
            const orbit=q('.mesh-orbit')?.getBoundingClientRect();
            ...
            return {
              ...
              boundaryColumns:columns('.boundary-list'),
              ...
              orbitRight:orbit?.right||0,
              orbitLeft:orbit?.left||0,
        ...
        require(isinstance(state, dict), f"Mesh Center layout state unreadable at {requested_width}px: {state!r}")
        width = int(state.get("width", requested_width))
        require(state.get("ready") == "complete", f"Mesh Center did not finish loading at {width}px: {state}")
        require(int(state.get("scrollWidth", width + 2)) <= width + 1, f"Mesh Center has horizontal overflow at {width}px: {state}")
        require(float(state.get("minNavHeight", 0)) >= 47.5, f"Mesh Center nav target below 48px at {width}px: {state}")
        require(float(state.get("minThemeHeight", 0)) >= 47.5 and float(state.get("minThemeWidth", 0)) >= 47.5, f"Mesh Center appearance control below 48px at {width}px: {state}")
        require(float(state.get("orbitLeft", 0)) >= -1 and float(state.get("orbitRight", width + 1)) <= width + 1, f"Mesh Center conceptual orbit escapes viewport at {width}px: {state}")

        expected_cap = 2 if width > 720 else 1
        expected_principle = 3 if width > 980 else (2 if width > 720 else 1)
        expected_boundary = 2 if width > 720 else 1
        expected_status = 2 if width > 720 else 1
        require(int(state.get("capColumns", 0)) == expected_cap, f"Mesh Center capability grid is not {expected_cap} columns at {width}px: {state}")
        require(int(state.get("principleColumns", 0)) == expected_principle, f"Mesh Center evidence grid is not {expected_principle} columns at {width}px: {state}")
        require(int(state.get("boundaryColumns", 0)) == expected_boundary, f"Mesh Center authority grid is not {expected_boundary} columns at {width}px: {state}")
        require(int(state.get("statusColumns", 0)) == expected_status, f"Mesh Center status grid is not {expected_status} columns at {width}px: {state}")
        expected_header = "sticky" if width > 720 else "relative"
        require(state.get("headerPosition") == expected_header, f"Mesh Center header position is not {expected_header} at {width}px: {state}")

        if width <= 480:
            actions = state.get("actions") or []
            container_width = float(state.get("actionContainerWidth", 0))
            require(bool(actions) and container_width > 0, f"Mesh Center actions missing at {width}px")
            require(all(float(action.get("width", 0)) >= container_width * 0.98 for action in actions), f"Mesh Center actions are not full-width at {width}px: {state}")

        images = state.get("images") or []
        failed = [image.get("src") for image in images if not image.get("complete") or int(image.get("naturalWidth", 0)) <= 0 or int(image.get("naturalHeight", 0)) <= 0]
        require(not failed, f"Mesh Center contains failed image loads at {width}px: {failed}")
    ...
    require(isinstance(theme_state, dict), f"Mesh Center appearance state unreadable: {theme_state!r}")
    require(theme_state.get("light") == "light" and theme_state.get("dark") == "dark", f"Mesh Center Light/Dark controls failed: {theme_state}")
    require(theme_state.get("system") is None and theme_state.get("selected") == ["system"], f"Mesh Center System appearance reset failed: {theme_state}")


def main() -> int:
    require(DIST.is_dir() and (DIST / "index.html").is_file(), "Mesh Center dist/ artifact is missing; run build first")
    ...
    try:
        ...
        wait_http(TARGET)
        with tempfile.NamedTemporaryFile(prefix="mesh-chromedriver-", suffix=".log", delete=False) as log:
            log_path = log.name
            driver = subprocess.Popen([chromedriver(), f"--port={DRIVER_PORT}", "--allowed-ips=127.0.0.1"], stdout=log, stderr=subprocess.STDOUT)
        wait_driver()
        session_id = create_session()
        exercise(session_id)
        print("Mesh Center responsive Chrome smoke passed at 1180, 768, 390, and 320px against the built public artifact, including System/Light/Dark controls.")
        return 0
    except Exception as error:
        print(f"Mesh Center responsive Chrome smoke failed: {error}")
        if log_path:
            try:
