#!/usr/bin/env python3
"""Exercise the live GoreeCloud Suite GLAZE UI V1.4 publication in headless Chrome."""
...
def verify_live_content(session_id: str) -> None:
    ...
    require(state.get("glaze") == EXPECTED_GLAZE_VERSION, f"Suite production browser Glaze version mismatch: {state}")
    require(state.get("glazeRevision") == EXPECTED_GLAZE_REVISION, f"Suite production browser Glaze revision mismatch: {state}")
    require(state.get("consumerState") == "build-migrated-rendered-acceptance-pending", f"Suite consumer-state mismatch: {state}")
    ...
    for marker in ("45\nverified Suite products", "9\nfunctional product groups", "GLAZE UI V1.4"):
...
def main() -> int:
    ...
    try:
        with tempfile.NamedTemporaryFile(prefix="suite-production-chromedriver-", suffix=".log", delete=False) as log:
            log_path = log.name
            driver = subprocess.Popen([smoke.chromedriver(), f"--port={smoke.DRIVER_PORT}", "--allowed-ips=127.0.0.1"], stdout=log, stderr=subprocess.STDOUT)
        smoke.wait_driver()
        ...
        print(
            ...
            "exact GLAZE UI V1.4 identity, representative reconciled products, canonical URL, and all images loaded."
        )
        return 0
